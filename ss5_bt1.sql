...
SELECT * FROM orders
WHERE oTotalPrice BETWEEN 150001 AND 300000;

CREATE INDEX idx_customer_name_age ON customer(cName, cAge);

CREATE INDEX idx_product_name_price ON products(pName, pPrice);
...
BEGIN
    ...
    SELECT MIN(oTotalPrice) INTO min_price FROM orders;

    SELECT * FROM orders
    WHERE oTotalPrice = min_price;
END //
...
BEGIN
    ...
    SELECT MIN(total_quantity) INTO min_quantity
    FROM (
    ...
    WHERE subquery.total_quantity = min_quantity;
END //
