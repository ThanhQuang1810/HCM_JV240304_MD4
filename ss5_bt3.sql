...
BEGIN
    ...
    WHERE EXISTS (
        SELECT 1
        FROM mark m
        WHERE m.subject_id = s.subject_id AND m.point = 10
    );
END //
...
BEGIN
    ...
    WHERE EXISTS (
        ...
        WHERE c.class_id = s.class_id AND m.point = 10
    );
END //
...
BEGIN
    ...
    SELECT new_id INTO p_student_id;
END //
